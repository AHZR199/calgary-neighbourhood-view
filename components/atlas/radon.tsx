import { ArrowUpRight, BookOpen, ChevronRight, House } from 'lucide-react';
    ...
    return (
      <section className="overview-topic radon-section">
        ...
        <p className="location-origin">
          Calgary metropolitan area · Health Canada ·{' '}
          {CALGARY_RADON.surveyPeriod}
        </p>
        <p className="quiet-note">
          <strong>{CALGARY_RADON.percentageAbove.toFixed(1)}%</strong> of this
          historical sample was above {RADON_GUIDELINE} Bq/m³ (
          {CALGARY_RADON.aboveGuideline} of {CALGARY_RADON.sampleSize}{' '}
          readings). This cannot tell you the level in a home or neighbourhood.
        </p>
        <a
          ...
          rel="noreferrer"
        >
          Test the home itself · Health Canada guidance{' '}
          <ArrowUpRight size={13} />
        </a>
        <details className="reading-detail">
          <summary>
            Radon research & testing guidance <ChevronRight size={14} />
          </summary>
          <RadonResearch onSource={onSource} />
        </details>
        <p className="data-attribution">
          ...
          . Calculations by this app.
        </p>
      </section>
    );
  return <RadonResearch onSource={onSource} />;
}

function RadonResearch({ onSource }: { onSource?: (id: string) => void }) {
  return (
    <section className="overview-topic radon-section">
      <div className="section-line">
        <h3>Indoor radon</h3>
      ...
      <p className="quiet-note">
        ...
        Health Canada recommends reducing it within one year, sooner for higher
        readings. The guideline is an action level; lower readings do not mean
        zero risk.
      </p>
      <details className="access-method">
        <summary>What this survey can tell you</summary>
        <p>
          These are {CALGARY_RADON.sampleSize} published Calgary-region readings
          from {CALGARY_RADON.surveyPeriod}, counted equally. Tests lasted{' '}
          {CALGARY_RADON.testDurationDays.minimum}–
          {CALGARY_RADON.testDurationDays.maximum} days;{' '}
          {CALGARY_RADON.testDurationDays.atLeast90Days} ran for at least 90
          days. The sample is dated and covers the wider metropolitan area. It
          is not a current citywide prevalence estimate.
        </p>
        <p>
          Postal-area samples are too small to support useful neighbourhood
          comparisons. This app does not assign a radon score to an address, or
          infer indoor radon from outdoor air quality, construction year or
          nearby measurements.
        </p>
